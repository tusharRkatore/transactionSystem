...
transactionRoute.get("/combined-data", async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      fetch(`http://localhost:5008/statistics?month=${month}`).then((res) =>
        res.json()
      ),
      fetch(`http://localhost:5008/price-distribution?month=${month}`).then(
        (res) => res.json()
      ),
      fetch(`http://localhost:5008/category-distribution?month=${month}`).then(
        (res) => res.json()
      ),
    ]);
